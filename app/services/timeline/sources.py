...
def rows(db, sql, patient_id, module_id=None, module_column=None, limit=None, order_by=None, record_ids=None):
    params = {'patient_id': patient_id}
    expanding = []
    if isinstance(patient_id, (list, tuple)):
        if not patient_id:
            return []
        sql = sql.replace('=:patient_id', ' IN :patient_id')
        expanding.append('patient_id')
    if module_id is not None:
        sql += ' AND ' + module_column + '=:module_id'
        params['module_id'] = module_id
    if record_ids is not None:
        if not record_ids:
            return []
        sql += ' AND r.id IN :record_ids'
        params['record_ids'] = record_ids
        expanding.append('record_ids')
    if limit is not None:
        sql += ' ORDER BY ' + order_by + ' LIMIT :source_limit'
        params['source_limit'] = limit
    statement = text(sql)
...
def daily_records(db, patient_id, registry, module_id=None, limit=None, recent_activity=False):
    ...
    for r in records:
        ...
        metadata = {'module_id': r['modulo_id'], 'form_id': r['formulario_id'],
                    'answers': grouped.get(r['id'], []), 'observacoes': r.get('observacoes')}
        if len(actors) > 1:
            metadata['source_actors'] = actors
        result.append(TimelineEvent(S.LONGITUDINAL_RECORD, r['id'], r.get('patient_id', patient_id),
...
def sessions(db, patient_id, registry, module_id=None, limit=None, recent_activity=False):
    ...
    for r in rows(db, '''SELECT s.paciente_id AS patient_id, s.id, s.data_realizacao, s.hora_fim_real, s.created_at,
        ...
        result.append(TimelineEvent(S.ASSISTENTIAL_SESSION, r['id'], r.get('patient_id', patient_id),
