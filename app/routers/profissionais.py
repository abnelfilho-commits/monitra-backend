...
def listar_profissionais(
    ...
    query = db.query(Profissional).filter(
    ...
    if not is_admin_global(usuario_atual):
        ...
        query = query.filter(
            Profissional.clinica_id == usuario_atual.clinica_id
        )
...
def listar_profissionais_por_clinica(
    ...
    if (
        ...
        and usuario.clinica_id != clinica_id
    ):
        raise HTTPException(
            status_code=403,
...
def obter_profissional(
    ...
    if (
        not is_admin_global(usuario)
        and usuario.clinica_id != profissional.clinica_id
    ):
        raise HTTPException(
            status_code=403,
            detail="Acesso negado",
...
def criar_profissional(
    ...
    try:
        ...
        return serializar_profissional(
            novo_profissional
        )
...
def atualizar_profissional(
    ...
    if (
        is_admin_global(usuario)
        and payload.clinica_id is not None
    ):
