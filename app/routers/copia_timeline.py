...
def obter_timeline(paciente_id: int, db: Session = Depends(get_db)):
    ...
    rows = db.execute(
        text("""
            ...
            SELECT
                rd.id,
                rd.paciente_id,
                'REGISTRO_DIARIO' AS tipo_evento,
                rd.data,
                rd.observacao,
                COALESCE(rd.origem, 'PROFISSIONAL') AS origem
            FROM registros_diarios rd
            WHERE rd.paciente_id = :paciente_id
