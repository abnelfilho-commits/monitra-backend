...
- Auditoria de Tendência no prontuário Cardio (paciente 12; quatro registros em 08/09, 15/09, 18/09 e 22/09/2026 informados pelo usuário): read_cardio seleciona somente a observação mais recente e retorna trend=None por decisão clínica aprovada; metadata.trend_availability=unavailable_pending_clinical_validation. patient_response expõe tendencia=reading.trend e PacienteCardiometabolico apresenta Indisponível. A evolução de medições é independente. Evidências: app/services/clinical_reading/providers/cardio.py:21,51,101; app/services/cardio_longitudinal.py:36; FE src/pages/cardiometabolico/PacienteCardiometabolico.jsx:207,312; tests/test_clinical_reading.py:315. Sem acesso HML ou validação dos registros nesta auditoria. Não classificado como bug de persistência nem reaberto CARDIO-005. Proposta mínima: explicar na apresentação que a tendência clínica aguarda validação; ativar cálculo exige contrato clínico específico aprovado (janelas, entradas e critérios), sem promover automaticamente algoritmo legado. Nenhuma implementação autorizada por este registro.
