...
class ModelContractTests(unittest.TestCase):
    def test_all_mapped_columns_match_physical_contract(self):
        ...
        for name, table in Base.metadata.tables.items():
            if name in {"capacidade_instalada", "usuario_instituicao_acessos", "institucional_operacoes", "identidade_operacoes", "pessoas", "instituicoes", "instituicao_papeis", "paciente_instituicoes", "profissional_instituicoes", "paciente_profissionais"}:
    ...
    def test_separate_single_heads(self):
        self.assertEqual(ScriptDirectory.from_config(config()).get_heads(), ["g2c1_autorizacao_v1"])
